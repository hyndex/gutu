...
- You need **charts**, **visual analytics**, **dashboard primitives** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/chart` instead of reaching into app-specific internals.
...
| `framework/libraries/chart` | Nested publishable library package. |
| `framework/libraries/chart/src` | Public runtime source and exported modules. |
| `framework/libraries/chart/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `buildCartesianChartOption`, `createLineChartOption`, `createBarChartOption`, `createStackedBarChartOption`, `createPieChartOption` |
...
- Hosts should import from `@platform/chart`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/chart"]
...
console.log(typeof buildCartesianChartOption);
```
