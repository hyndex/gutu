...
| Field | Value |
| --- | --- |
| Package Name | `@platform/admin-formview` |
...
| Named Exports | 15 | `packageId`, `packageDisplayName`, `packageDescription`, `defineFormView`, `defineDetailView`, `createDirtyStateGuard`, `createAsyncFieldValidationAdapter`, `createFormViewFromResource` |
...
console.log(packageId, packageDisplayName, typeof defineFormView);
```

Use the root repo scripts for day-to-day work after the workspace is bootstrapped, or run the nested package directly from `framework/libraries/admin-formview` if you need lower-level control.
