...
export function createListViewFromResource(resource: ResourceDefinition): ListViewDefinition {
  const fieldConfig = resource.fields ?? {};
  const defaultColumns = resource.admin?.defaultColumns ?? Object.keys(fieldConfig);

  return defineListView({
    id: `${resource.id}.auto-list`,
    resource: resource.id,
    columns: defaultColumns.map((field) => ({
      key: field,
      label: fieldConfig[field]?.label ?? field,
      sortable: fieldConfig[field]?.sortable ?? false,
      searchable: fieldConfig[field]?.searchable ?? false
    })),
    filters: Object.entries(fieldConfig)
      .filter(([, config]) => typeof config.filter === "string")
      .map(([field, config]) => ({
        key: field,
        type: (config.filter ?? "text") as ListFilterDefinition["type"]
      })),
    bulkActions: [],
