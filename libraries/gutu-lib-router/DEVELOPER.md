...
Canonical router wrapper with admin guards and safe zone links.
...
- You need **routing contracts**, **navigation helpers**, **URL semantics** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/router` instead of reaching into app-specific internals.
...
| `framework/libraries/router` | Nested publishable library package. |
| `framework/libraries/router/src` | Public runtime source and exported modules. |
| `framework/libraries/router/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `assertRouteAccess`, `createZoneSafeHref`, `RouteAccessContext` |
...
- Hosts should import from `@platform/router`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/router"]
...
```ts
import { packageId, packageDisplayName, assertRouteAccess } from "@platform/router";

console.log(packageId, packageDisplayName);
console.log(typeof assertRouteAccess);
```

### 2. Safe consumption pattern

- Import from the package entrypoint, not `src/` internals.
- Compose through documented modules such as `@platform/ui-router`.
...
- Publishes 1 public module from `@platform/router`: `@platform/ui-router`.
- Exports 6 named symbols through the public entrypoint, including `packageId`, `packageDisplayName`, `packageDescription`, `assertRouteAccess`, `createZoneSafeHref`, `RouteAccessContext`.
