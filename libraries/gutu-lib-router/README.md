...
| --- | --- | --- |
| Public Modules | 1 | `@platform/ui-router` |
| Named Exports | 6 | `packageId`, `packageDisplayName`, `packageDescription`, `assertRouteAccess`, `createZoneSafeHref`, `RouteAccessContext` |
...
console.log(packageId, packageDisplayName, typeof assertRouteAccess);
```

Use the root repo scripts for day-to-day work after the workspace is bootstrapped, or run the nested package directly from `framework/libraries/router` if you need lower-level control.
