...
| Named Exports | 21 | `packageId`, `packageDisplayName`, `packageDescription`, `defineSearchDocument`, `defineSearchIndex`, `tokenizeSearchQuery`, `normalizeSearchQuery`, `buildPostgresTsQuery` |
...
console.log(packageId, packageDisplayName, typeof defineSearchDocument);
```

Use the root repo scripts for day-to-day work after the workspace is bootstrapped, or run the nested package directly from `framework/libraries/search` if you need lower-level control.
