...
- You need **search helpers**, **result contracts**, **query composition** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/search` instead of reaching into app-specific internals.
...
| `framework/libraries/search` | Nested publishable library package. |
| `framework/libraries/search/src` | Public runtime source and exported modules. |
| `framework/libraries/search/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `defineSearchDocument`, `defineSearchIndex`, `tokenizeSearchQuery`, `normalizeSearchQuery`, `buildPostgresTsQuery` |
...
- Hosts should import from `@platform/search`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/search"]
...
```ts
import { packageId, packageDisplayName, defineSearchDocument } from "@platform/search";

console.log(packageId, packageDisplayName);
console.log(typeof defineSearchDocument);
```
...
- Publishes 0 public modules from `@platform/search`.
- Exports 21 named symbols through the public entrypoint, including `packageId`, `packageDisplayName`, `packageDescription`, `defineSearchDocument`, `defineSearchIndex`, `tokenizeSearchQuery`, and more.
