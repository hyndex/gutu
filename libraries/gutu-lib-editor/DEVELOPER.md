...
Editor reusable library exports.
...
- You need **editor primitives**, **rich editing**, **shared authoring UI** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/editor` instead of reaching into app-specific internals.
...
| `framework/libraries/editor` | Nested publishable library package. |
| `framework/libraries/editor/src` | Public runtime source and exported modules. |
| `framework/libraries/editor/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.tsx` | `packageId`, `packageDisplayName`, `packageDescription`, `createAdminEditorPreset`, `createReadOnlyEditorPreset`, `renderReadOnlyEditorContent`, `ReadOnlyEditorRenderer`, `PlatformEditorExtensionSeams` |
...
- Hosts should import from `@platform/editor`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/editor"]
...
```ts
import { packageId, packageDisplayName } from "@platform/editor";
