# UI Router Developer Guide

TanStack Router wrapper APIs.
...
- You need **router-aware UI**, **navigation components**, **front-end composition** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/ui-router` instead of reaching into app-specific internals.
...
| `framework/libraries/ui-router` | Nested publishable library package. |
| `framework/libraries/ui-router/src` | Public runtime source and exported modules. |
| `framework/libraries/ui-router/tests` | Unit and contract verification where present. |
...
| Field | Value |
| --- | --- |
| Direct Dependencies | `@platform/kernel`, `@tanstack/react-router`, `react` |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `defineRoute`, `createRouteManifest`, `buildRouteHref`, `matchRoute`, `extractRouteParams` |
...
- Hosts should import from `@platform/ui-router`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/ui-router"]
...
```ts
import { packageId, packageDisplayName, defineRoute } from "@platform/ui-router";

console.log(packageId, packageDisplayName);
console.log(typeof defineRoute);
```
