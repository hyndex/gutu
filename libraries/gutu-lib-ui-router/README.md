...
| Field | Value |
| --- | --- |
| Package Name | `@platform/ui-router` |
...
| Named Exports | 15 | `packageId`, `packageDisplayName`, `packageDescription`, `defineRoute`, `createRouteManifest`, `buildRouteHref`, `matchRoute`, `extractRouteParams` |
...
console.log(packageId, packageDisplayName, typeof defineRoute);
```

Use the root repo scripts for day-to-day work after the workspace is bootstrapped, or run the nested package directly from `framework/libraries/ui-router` if you need lower-level control.
