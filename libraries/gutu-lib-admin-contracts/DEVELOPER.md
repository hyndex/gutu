# Admin Contracts Developer Guide
...
- You need **registry contracts**, **admin access**, **legacy adapters** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/admin-contracts` instead of reaching into app-specific internals.
...
| `framework/libraries/admin-contracts` | Nested publishable library package. |
| `framework/libraries/admin-contracts/src` | Public runtime source and exported modules. |
| `framework/libraries/admin-contracts/tests` | Unit and contract verification where present. |
...
| --- | --- | --- |
| `./access` | `src/access.ts` | `canViewPage`, `canRunAction`, `canSeeField`, `canSeeWidget`, `canViewReport`, `canUseBuilder`, `canLaunchZone`, `canUseCommand` |
| `./legacy` | `src/legacy.ts` | `adaptLegacyUiRegistry` |
| `./registry` | `src/registry.ts` | `defineWorkspace`, `defineAdminNav`, `definePage`, `defineWidget`, `defineReport`, `defineCommand`, `defineSearchProvider`, `defineBuilder` |
| `./types` | `src/types.ts` | `AdminPageKind`, `WidgetKind`, `ReportKind`, `BuilderMode`, `FieldVisibilityRule`, `WorkspaceContribution`, `AdminNavItem`, `AdminNavContribution` |

### Source module map

| Source File | Exported Symbols |
| --- | --- |
| `access.ts` | `canViewPage`, `canRunAction`, `canSeeField`, `canSeeWidget`, `canViewReport`, `canUseBuilder`, `canLaunchZone`, `canUseCommand` |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription` |
| `legacy.ts` | `adaptLegacyUiRegistry` |
| `registry.ts` | `defineWorkspace`, `defineAdminNav`, `definePage`, `defineWidget`, `defineReport`, `defineCommand`, `defineSearchProvider`, `defineBuilder` |
| `types.ts` | `AdminPageKind`, `WidgetKind`, `ReportKind`, `BuilderMode`, `FieldVisibilityRule`, `WorkspaceContribution`, `AdminNavItem`, `AdminNavContribution` |
...
- Hosts should import from `@platform/admin-contracts`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/admin-contracts"]
...
console.log(typeof canViewPage);
```

### 2. Safe consumption pattern

- Import from the package entrypoint, not `src/` internals.
- Compose through documented modules such as `./access`, `./legacy`, `./registry`, `./types`.
