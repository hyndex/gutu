...
Durable agent runtime contracts, checkpoints, replay, and budget enforcement.
...
- You need **runtime state**, **execution helpers**, **AI orchestration primitives** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/ai-runtime` instead of reaching into app-specific internals.
...
| `framework/libraries/ai-runtime` | Nested publishable library package. |
| `framework/libraries/ai-runtime/src` | Public runtime source and exported modules. |
| `framework/libraries/ai-runtime/tests` | Unit and contract verification where present. |

## Package Contract

| Field | Value |
| --- | --- |
| Package ID | `ai-runtime` |
| Display Name | AI Runtime |
| Import Name | `@platform/ai-runtime` |
| Version | `0.1.0` |
...
| --- | --- | --- |
| `./state` | `src/state.ts` | `resolveStateDirectory`, `resolveStateFile`, `loadJsonState`, `saveJsonState`, `updateJsonState` |

### Source module map

| Source File | Exported Symbols |
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `defineAgent`, `createAgentRunRecord`, `appendAgentStep`, `pauseAgentRunForApproval`, `approveCheckpoint` |
| `state.ts` | `resolveStateDirectory`, `resolveStateFile`, `loadJsonState`, `saveJsonState`, `updateJsonState` |
...
- Hosts should import from `@platform/ai-runtime`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/ai-runtime"]
...
```ts
import { packageId, packageDisplayName, defineAgent } from "@platform/ai-runtime";

console.log(packageId, packageDisplayName);
console.log(typeof defineAgent);
```

### 2. Safe consumption pattern

- Import from the package entrypoint, not `src/` internals.
- Compose through documented modules such as `./state`.
...
- Publishes 1 public module from `@platform/ai-runtime`: `./state`.
- Exports 40 named symbols through the public entrypoint, including `packageId`, `packageDisplayName`, `packageDescription`, `defineAgent`, `createAgentRunRecord`, `appendAgentStep`, and more.
