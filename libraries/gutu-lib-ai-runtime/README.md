...
| Field | Value |
| --- | --- |
| Package Name | `@platform/ai-runtime` |
...
| --- | --- | --- |
| Public Modules | 1 | `./state` |
| Named Exports | 40 | `packageId`, `packageDisplayName`, `packageDescription`, `defineAgent`, `createAgentRunRecord`, `appendAgentStep`, `pauseAgentRunForApproval`, `approveCheckpoint` |
...
console.log(packageId, packageDisplayName, typeof defineAgent);
```

Use the root repo scripts for day-to-day work after the workspace is bootstrapped, or run the nested package directly from `framework/libraries/ai-runtime` if you need lower-level control.
