...
- You need **form components**, **interaction primitives**, **shared inputs** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/ui-form` instead of reaching into app-specific internals.
...
| `framework/libraries/ui-form` | Nested publishable library package. |
| `framework/libraries/ui-form/src` | Public runtime source and exported modules. |
| `framework/libraries/ui-form/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `usePlatformForm`, `createFormDefaults`, `mapSubmissionErrors`, `createFieldRegistry`, `createRelationFieldAdapter` |
...
- Hosts should import from `@platform/ui-form`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/ui-form"]
...
console.log(typeof usePlatformForm);
```
