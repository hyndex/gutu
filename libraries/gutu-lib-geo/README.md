...
| Named Exports | 15 | `packageId`, `packageDisplayName`, `packageDescription`, `defineGeoProvider`, `haversineDistanceKm`, `calculateBoundingBox`, `geocode`, `reverseGeocode` |
...
console.log(packageId, packageDisplayName, typeof defineGeoProvider);
```

Use the root repo scripts for day-to-day work after the workspace is bootstrapped, or run the nested package directly from `framework/libraries/geo` if you need lower-level control.
