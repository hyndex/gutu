...
Geo abstraction and provider contracts.
...
- You need **geographic helpers**, **location utilities**, **typed coordinates** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/geo` instead of reaching into app-specific internals.
...
| `framework/libraries/geo` | Nested publishable library package. |
| `framework/libraries/geo/src` | Public runtime source and exported modules. |
| `framework/libraries/geo/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `defineGeoProvider`, `haversineDistanceKm`, `calculateBoundingBox`, `geocode`, `reverseGeocode` |
...
- Hosts should import from `@platform/geo`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/geo"]
...
```ts
import { packageId, packageDisplayName, defineGeoProvider } from "@platform/geo";

console.log(packageId, packageDisplayName);
console.log(typeof defineGeoProvider);
```
...
- Publishes 0 public modules from `@platform/geo`.
- Exports 15 named symbols through the public entrypoint, including `packageId`, `packageDisplayName`, `packageDescription`, `defineGeoProvider`, `haversineDistanceKm`, `calculateBoundingBox`, and more.
