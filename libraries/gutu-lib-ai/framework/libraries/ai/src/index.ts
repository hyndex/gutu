...
export function createToolContract(action: ActionDefinition, description = action.id): ToolContract {
  ...
  const policies = normalizeToolPolicies((aiMetadata?.toolPolicies ?? []) as ToolPolicy[], {
  ...
  return {
    ...
    permission: action.permission ?? action.id,
    idempotent: action.idempotent ?? false,
    audit: action.audit ?? false,
    ...
    replay: {
      deterministic: aiMetadata?.replay?.deterministic ?? action.idempotent ?? false,
      includeInputHash: aiMetadata?.replay?.includeInputHash ?? true,
      includeOutputHash: aiMetadata?.replay?.includeOutputHash ?? action.audit ?? false,
