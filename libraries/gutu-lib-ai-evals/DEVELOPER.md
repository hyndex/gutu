...
- You need **evaluation helpers**, **baseline comparisons**, **AI verification** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/ai-evals` instead of reaching into app-specific internals.
...
| `framework/libraries/ai-evals` | Nested publishable library package. |
| `framework/libraries/ai-evals/src` | Public runtime source and exported modules. |
| `framework/libraries/ai-evals/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `defineEvalDataset`, `runEvalDataset`, `createEvalBaseline`, `compareEvalRuns`, `checkRegressionGate` |
...
- Hosts should import from `@platform/ai-evals`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/ai-evals"]
...
console.log(typeof defineEvalDataset);
```
