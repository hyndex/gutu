...
- You need **memory helpers**, **retrieval state**, **AI persistence seams** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/ai-memory` instead of reaching into app-specific internals.
...
| `framework/libraries/ai-memory` | Nested publishable library package. |
| `framework/libraries/ai-memory/src` | Public runtime source and exported modules. |
| `framework/libraries/ai-memory/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `defineMemoryCollection`, `defineMemoryDocument`, `defineMemoryPolicy`, `chunkMemoryDocument`, `buildRetrievalPlan` |
...
- Hosts should import from `@platform/ai-memory`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/ai-memory"]
...
console.log(typeof defineMemoryCollection);
```
