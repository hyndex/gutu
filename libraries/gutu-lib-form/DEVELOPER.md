...
Canonical form wrapper with field access, autosave, and relation helpers.
...
- You need **form helpers**, **schema state**, **input composition** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/form` instead of reaching into app-specific internals.
...
| `framework/libraries/form` | Nested publishable library package. |
| `framework/libraries/form/src` | Public runtime source and exported modules. |
| `framework/libraries/form/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `resolveFieldAccess`, `createAutosaveController`, `PermissionAwareFieldRule`, `PermissionAwareFieldState`, `AutosaveController` |
...
- Hosts should import from `@platform/form`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/form"]
...
```ts
import { packageId, packageDisplayName, resolveFieldAccess } from "@platform/form";

console.log(packageId, packageDisplayName);
console.log(typeof resolveFieldAccess);
```

### 2. Safe consumption pattern

- Import from the package entrypoint, not `src/` internals.
- Compose through documented modules such as `@platform/ui-form`.
...
- Publishes 1 public module from `@platform/form`: `@platform/ui-form`.
- Exports 8 named symbols through the public entrypoint, including `packageId`, `packageDisplayName`, `packageDescription`, `resolveFieldAccess`, `createAutosaveController`, `PermissionAwareFieldRule`, and more.
