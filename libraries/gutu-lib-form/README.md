...
| --- | --- | --- |
| Public Modules | 1 | `@platform/ui-form` |
| Named Exports | 8 | `packageId`, `packageDisplayName`, `packageDescription`, `resolveFieldAccess`, `createAutosaveController`, `PermissionAwareFieldRule`, `PermissionAwareFieldState`, `AutosaveController` |
...
console.log(packageId, packageDisplayName, typeof resolveFieldAccess);
```

Use the root repo scripts for day-to-day work after the workspace is bootstrapped, or run the nested package directly from `framework/libraries/form` if you need lower-level control.
