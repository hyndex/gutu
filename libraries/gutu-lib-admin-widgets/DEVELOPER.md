# Admin Widgets Developer Guide

Admin Widgets reusable library exports.
...
- You need **widgets**, **summary cards**, **admin composition** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/admin-widgets` instead of reaching into app-specific internals.
...
| `framework/libraries/admin-widgets` | Nested publishable library package. |
| `framework/libraries/admin-widgets/src` | Public runtime source and exported modules. |
| `framework/libraries/admin-widgets/tests` | Unit and contract verification where present. |
...
| --- | --- | --- |
| `./main` | `src/main.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `defineDashboard`, `defineSavedQuestion`, `KpiCard`, `ChartCard`, `ActivityFeed` |
...
| `main.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `defineDashboard`, `defineSavedQuestion`, `KpiCard`, `ChartCard`, `ActivityFeed` |
...
- Hosts should import from `@platform/admin-widgets`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/admin-widgets"]
...
```ts
import { packageId, packageDisplayName, defineDashboard } from "@platform/admin-widgets";

console.log(packageId, packageDisplayName);
console.log(typeof defineDashboard);
```
...
- Publishes 1 public module from `@platform/admin-widgets`: `./main`.
- Exports 16 named symbols through the public entrypoint, including `packageId`, `packageDisplayName`, `packageDescription`, `defineDashboard`, `defineSavedQuestion`, `KpiCard`, and more.
