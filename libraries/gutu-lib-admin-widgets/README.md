...
| Named Exports | 16 | `packageId`, `packageDisplayName`, `packageDescription`, `defineDashboard`, `defineSavedQuestion`, `KpiCard`, `ChartCard`, `ActivityFeed` |
...
console.log(packageId, packageDisplayName, typeof defineDashboard);
```

Use the root repo scripts for day-to-day work after the workspace is bootstrapped, or run the nested package directly from `framework/libraries/admin-widgets` if you need lower-level control.
