...
- You need **data tables**, **table state**, **list helpers** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/data-table` instead of reaching into app-specific internals.
...
| `framework/libraries/data-table` | Nested publishable library package. |
| `framework/libraries/data-table/src` | Public runtime source and exported modules. |
| `framework/libraries/data-table/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `createVirtualWindowState`, `getVirtualRows`, `usePlatformVirtualRows`, `createSavedViewSnapshot`, `applySavedViewSnapshot` |
...
- Hosts should import from `@platform/data-table`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/data-table"]
...
console.log(typeof createVirtualWindowState);
```

### 2. Safe consumption pattern

- Import from the package entrypoint, not `src/` internals.
- Compose through documented modules such as `@platform/ui-table`.
