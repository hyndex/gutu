...
- You need **telemetry helpers**, **UI metrics**, **front-end observability** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/telemetry-ui` instead of reaching into app-specific internals.
...
| `framework/libraries/telemetry-ui` | Nested publishable library package. |
| `framework/libraries/telemetry-ui/src` | Public runtime source and exported modules. |
| `framework/libraries/telemetry-ui/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `createUiTelemetryEvent`, `trackPageView`, `trackUiAction`, `trackWidgetView`, `trackCommandPalette` |
...
- Hosts should import from `@platform/telemetry-ui`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/telemetry-ui"]
...
console.log(typeof createUiTelemetryEvent);
```
