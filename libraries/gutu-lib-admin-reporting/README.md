...
| Field | Value |
| --- | --- |
| Package Name | `@platform/admin-reporting` |
...
| Named Exports | 8 | `packageId`, `packageDisplayName`, `packageDescription`, `createSemanticQueryDefinition`, `createReportExecutionRequest`, `validateReportFilterInput`, `SemanticQueryDefinition`, `ReportExecutionRequest` |
...
console.log(packageId, packageDisplayName, typeof createSemanticQueryDefinition);
```

Use the root repo scripts for day-to-day work after the workspace is bootstrapped, or run the nested package directly from `framework/libraries/admin-reporting` if you need lower-level control.
