# Admin Reporting Developer Guide

Report contracts, filter validation, and semantic execution requests.
...
- You need **reporting helpers**, **operator summaries**, **admin analytics** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/admin-reporting` instead of reaching into app-specific internals.
...
| `framework/libraries/admin-reporting` | Nested publishable library package. |
| `framework/libraries/admin-reporting/src` | Public runtime source and exported modules. |
| `framework/libraries/admin-reporting/tests` | Unit and contract verification where present. |

## Package Contract

| Field | Value |
| --- | --- |
| Package ID | `admin-reporting` |
| Display Name | Admin Reporting |
| Import Name | `@platform/admin-reporting` |
| Version | `0.1.0` |
...
| Field | Value |
| --- | --- |
| Direct Dependencies | `@platform/admin-contracts`, `@platform/search`, `zod` |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `createSemanticQueryDefinition`, `createReportExecutionRequest`, `validateReportFilterInput`, `SemanticQueryDefinition`, `ReportExecutionRequest` |
...
- Hosts should import from `@platform/admin-reporting`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/admin-reporting"]
...
```ts
import { packageId, packageDisplayName, createSemanticQueryDefinition } from "@platform/admin-reporting";

console.log(packageId, packageDisplayName);
console.log(typeof createSemanticQueryDefinition);
```
