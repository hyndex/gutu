...
- You need **presentational components**, **visual system**, **shared styling** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/ui-kit` instead of reaching into app-specific internals.
...
| `framework/libraries/ui-kit` | Nested publishable library package. |
| `framework/libraries/ui-kit/src` | Public runtime source and exported modules. |
| `framework/libraries/ui-kit/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `cn`, `PageSection`, `MetricCard`, `ObjectHeader`, `FilterBar` |
...
- Hosts should import from `@platform/ui-kit`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/ui-kit"]
