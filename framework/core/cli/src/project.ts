...
function writeStarterApp(projectRoot: string, appId: string, pluginId: string) {
  ...
  const docPack = createUnderstandingDocPack({
    id: appId,
    displayName,
    description: `Starter runnable app host for the ${displayName} workspace.`,
    location: `apps/${appId}`,
    targetType: "app",
    dependsOn: [pluginId],
    providesCapabilities: [`${appId}.host`],
    requestedCapabilities: [`${pluginId}.manage`],
    resources: [],
    actions: [],
    workflows: []
  });
  ...
  for (const [relativePath, contents] of Object.entries(docPack)) {
    write(appDir, join("docs", relativePath), contents);
  }
}
