...
const componentHotspotReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const by = new Map<string, { component: string; total: number; open: number; avgAgeDays: number }>();
    const now = Date.now();
    const ageSum = new Map<string, number>();
    ...
    for (const i of issues) {
      ...
      r.total++;
      if (i.status !== "resolved" && i.status !== "closed") r.open++;
      if (i.createdAt) {
...
const mttrReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const issues = await fetchAll(resources, "issues.issue");
    const by = new Map<string, { priority: string; count: number; totalDays: number; avgDays: number }>();
    for (const i of issues) {
  ...
  async execute({ resources }) {
    ...
    return {
      ...
      chart: { kind: "line", label: "Velocity",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.week, value: r.closed })) },
    };
  },
};

const topReportersReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const issues = await fetchAll(resources, "issues.issue");
    const by = new Map<string, { reporter: string; kind: string; issues: number }>();
    for (const i of issues) {
...
const regressionReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const issues = await fetchAll(resources, "issues.issue");
    const by = new Map<string, { release: string; total: number; regressions: number; rate: number }>();
    for (const i of issues) {
      const r = str(i.foundInRelease, "untagged");
      const row = by.get(r) ?? { release: r, total: 0, regressions: 0, rate: 0 };
      row.total++;
      if (i.kind === "regression") row.regressions++;
      by.set(r, row);
    }
    const rows = [...by.values()]
...
const slaReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const issues = await fetchAll(resources, "issues.issue");
    const by = new Map<string, { priority: string; total: number; breached: number; rate: number }>();
    for (const i of issues) {
      const p = str(i.priority);
      const r = by.get(p) ?? { priority: p, total: 0, breached: 0, rate: 0 };
      r.total++;
      if (i.slaBreached) r.breached++;
      by.set(p, r);
    }
...
export const ISSUES_REPORTS: readonly ReportDefinition[] = [
