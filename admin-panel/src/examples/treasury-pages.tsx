...
function TreasuryCashPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      <PageHeader title="Cash position" description="Total liquidity across bank accounts." />
      <MetricGrid
        columns={4}
        metrics={[
      ...
      <Card>
        ...
        <CardContent>
          {snapshots.length > 0 ? (
            <BarChart
              data={snapshots.map((s) => ({ label: s.month, value: Math.round(s.totalUsd / 1000) }))}
              height={220}
              valueFormatter={(v) => `$${v}K`}
            />
          ) : (
            <EmptyState title="No balance data yet" />
          )}
        </CardContent>
      </Card>
    </div>
