...
const dailyDispatchReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const by = new Map<string, { technician: string; scheduled: number; inProgress: number; completed: number }>();
    for (const j of jobs) {
      ...
      r.scheduled++;
      if (j.status === "in_progress") r.inProgress++;
      if (j.status === "resolved") r.completed++;
      by.set(t, r);
    }
...
const techUtilizationReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const jobs = await fetchAll(resources, "field-service.job");
    const by = new Map<string, { technician: string; jobs: number; hours: number }>();
    for (const j of jobs) {
      const t = str(j.technician, "Unassigned");
    ...
    return {
      ...
      chart: { kind: "bar", label: "Hours by technician",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.technician, value: r.hours })) },
    };
  },
};

const firstTimeFixReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const jobs = await fetchAll(resources, "field-service.job");
    const by = new Map<string, { technician: string; completed: number; firstTime: number; rate: number }>();
    for (const j of jobs) {
      if (j.status !== "resolved") continue;
      const t = str(j.technician, "Unassigned");
      const r = by.get(t) ?? { technician: t, completed: 0, firstTime: 0, rate: 0 };
      r.completed++;
      if (num(j.visits) <= 1) r.firstTime++;
      by.set(t, r);
    }
...
const routeEfficiencyReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const jobs = await fetchAll(resources, "field-service.job");
    const today = new Date(); today.setHours(0, 0, 0, 0);
    const by = new Map<string, { technician: string; jobs: number; travelKm: number; onSiteHours: number }>();
    for (const j of jobs) {
      const d = j.scheduledAt ? new Date(str(j.scheduledAt)) : null;
      if (!d || Number.isNaN(d.getTime())) continue;
      d.setHours(0, 0, 0, 0);
...
const partsUsageReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    const by = new Map<string, { partSku: string; partName: string; qty: number; cost: number }>();
...
const slaComplianceReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const jobs = await fetchAll(resources, "field-service.job");
    const by = new Map<string, { priority: string; total: number; onTime: number; breached: number; rate: number }>();
    for (const j of jobs) {
      const p = str(j.priority);
      const r = by.get(p) ?? { priority: p, total: 0, onTime: 0, breached: 0, rate: 0 };
      r.total++;
      if (j.slaBreached) r.breached++;
      else r.onTime++;
      by.set(p, r);
    }
...
export const FIELD_SERVICE_REPORTS: readonly ReportDefinition[] = [
  dailyDispatchReport,
