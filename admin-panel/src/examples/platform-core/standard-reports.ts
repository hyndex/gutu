...
export const salesAnalyticsReport: ReportDefinition = {
  id: "sales-analytics",
  ...
  async execute({ filters, resources }): Promise<ReportResult> {
    ...
    const byMonth = new Map<
      string,
      { month: string; count: number; pipeline: number; won: number; lost: number }
    >();
    for (const d of filtered) {
      ...
      if (d.stage === "won") b.won += amt;
      else if (d.stage === "lost") b.lost += amt;
      else b.pipeline += amt;
      byMonth.set(k, b);
    }
    ...
    return {
      ...
      chart: {
        ...
        format: "currency",
        currency: "USD",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.month, value: r.won })),
      },
    };
  },
};
...
export const salesPersonReport: ReportDefinition = {
  id: "sales-person",
  ...
  async execute({ filters, resources }): Promise<ReportResult> {
    ...
    const byOwner = new Map<
      string,
      { owner: string; deals: number; amount: number; won: number; winRate: number }
    >();
    for (const d of filtered) {
      ...
      if (d.stage === "won") b.won += num(d.amount);
      byOwner.set(o, b);
    }
    ...
    return {
      ...
      chart: {
        ...
        format: "currency",
        currency: "USD",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.owner, value: r.won })),
      },
    };
  },
};
...
export const invoiceAgingReport: ReportDefinition = {
  id: "invoice-aging",
  label: "Invoice Aging",
  ...
  async execute({ filters, resources }): Promise<ReportResult> {
    ...
    const active = invoices.filter(
      (i) => !filters.status || i.status === filters.status,
    );
    const BUCKETS = [
    ...
    return {
      ...
      chart: {
        ...
        format: "currency",
        currency: "USD",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.bucket, value: r.outstanding })),
      },
    };
  },
};
...
export const supportSlaReport: ReportDefinition = {
  id: "support-sla",
  ...
  async execute({ filters, resources }): Promise<ReportResult> {
    ...
    const filtered = tickets.filter(
      (t) => !(filters.from || filters.to) || inDateRange(t.createdAt, filters.from, filters.to),
    );
    ...
    for (const t of filtered) {
      ...
      else if (t.status === "resolved") r.resolved++;
      else if (t.status === "closed") r.closed++;
    }
    return {
      columns: [
...
export const stockBalanceReport: ReportDefinition = {
  id: "stock-balance",
  label: "Stock Balance",
  ...
  async execute({ filters, resources }): Promise<ReportResult> {
    ...
    const rows = items
      ...
      .filter(
        (it) =>
          !filters.belowReorder || num(it.onHand) <= num(it.reorderPoint),
      )
      .map((it) => ({
        sku: str(it.sku),
        name: str(it.name),
    ...
    return {
      ...
      chart: {
        ...
        from: (rs) => {
          ...
          for (const r of rs as typeof rows) {
...
export const customerAcquisitionReport: ReportDefinition = {
  ...
  async execute({ filters, resources }): Promise<ReportResult> {
    ...
    return {
      ...
      chart: {
        ...
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.month, value: r.count })),
      },
    };
  },
};
...
export const auditActorReport: ReportDefinition = {
  id: "audit-actor",
  ...
  async execute({ filters, resources }): Promise<ReportResult> {
    ...
    return {
      ...
      chart: {
        ...
        from: (rs) => (rs as typeof rows).slice(0, 12).map((r) => ({ label: r.actor, value: r.total })),
      },
    };
  },
};
...
export const bookingTrendsReport: ReportDefinition = {
  id: "booking-trends",
  ...
  async execute({ filters, resources }): Promise<ReportResult> {
    ...
    const filtered = bookings.filter(
      (b) => !(filters.from || filters.to) || inDateRange(b.createdAt, filters.from, filters.to),
    );
    const byDay = new Map<string, { day: string; total: number; completed: number; cancelled: number }>();
    ...
    return {
      ...
      chart: {
        ...
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.day.slice(5), value: r.total })),
      },
    };
  },
};
...
export const STANDARD_REPORTS: readonly ReportDefinition[] = [
...
export const REPORT_MODULES: Record<string, readonly string[]> = {
