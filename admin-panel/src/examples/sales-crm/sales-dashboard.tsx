...
/* Reports */
async function fetchAll(resources: import("@/runtime/resourceClient").ResourceClient, r: string) {
  return (await resources.list(r, { page: 1, pageSize: 10_000 })).rows as Record<string, unknown>[];
}
const num = (v: unknown, d = 0) => (typeof v === "number" && !Number.isNaN(v) ? v : d);
const str = (v: unknown, d = "") => (typeof v === "string" ? v : d);
const monthKey = (d: Date) => `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`;

const salesAnalyticsReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const byMonth = new Map<string, { month: string; opened: number; won: number; lost: number; count: number }>();
    for (const d of deals) {
      ...
      b.count++; b.opened += amt;
      if (d.stage === "won") b.won += amt;
      if (d.stage === "lost") b.lost += amt;
      byMonth.set(k, b);
    }
...
const territoryVarianceReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    return {
      ...
      chart: { kind: "bar", label: "Attainment % by territory", format: "percent",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.name, value: r.attainment })) },
    };
  },
};

const partnerCommissionReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    return {
      ...
      chart: { kind: "bar", label: "Commission earned", format: "currency", currency: "USD",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.name, value: r.earned })) },
    };
  },
};

const creditBalanceReport: ReportDefinition = {
...
const quoteTrendsReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    return {
      ...
      chart: { kind: "line", label: "Quote value by month", format: "currency", currency: "USD",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.month, value: r.value })) },
    };
  },
};

const pricingUsageReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    const byType = new Map<string, { type: string; count: number; avgValue: number; total: number }>();
    for (const r of rs) {
      const t = str(r.discountType);
      const b = byType.get(t) ?? { type: t, count: 0, avgValue: 0, total: 0 };
      b.count++; b.total += num(r.discountValue);
      byType.set(t, b);
    }
    ...
    return {
      ...
      chart: { kind: "donut", label: "Rules by discount type",
        from: (rs2) => (rs2 as typeof rows).map((r) => ({ label: r.type, value: r.count })) },
    };
  },
};
