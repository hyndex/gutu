...
export const leadDetailsReport: ReportDefinition = {
  id: "crm-lead-details",
  label: "Lead Details",
  ...
  async execute({ filters, resources }): Promise<ReportResult> {
...
export const leadConversionTimeReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const bySource = new Map<
      string,
      { source: string; leads: number; totalHours: number; avgHours: number; medianHours: number }
    >();
    for (const l of converted) {
      const created = parseDate(l.createdAt);
      const conv = parseDate(l.convertedAt);
      if (!created || !conv) continue;
    ...
    for (const b of bySource.values()) b.avgHours = b.leads > 0 ? b.totalHours / b.leads : 0;
    ...
    return {
      ...
      chart: {
        ...
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.source, value: Math.round(r.avgHours) })),
      },
    };
  },
};
...
export const leadOwnerEfficiencyReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    for (const l of leads) {
      ...
      b.total++;
      if (l.status === "contacted") b.contacted++;
      if (l.status === "qualified") b.qualified++;
      if (l.status === "converted") b.converted++;
      if (l.status === "lost") b.lost++;
    ...
    return {
      ...
      chart: {
        ...
        format: "percent",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.owner, value: r.conversionPct })),
      },
    };
  },
};
...
export const opportunitySummaryReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const byStage = new Map<
      string,
      { stage: string; count: number; amount: number; weighted: number; avgProbability: number }
    >();
...
export const pipelineAnalyticsReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const byMonth = new Map<
      string,
      { month: string; opened: number; won: number; lost: number; netPipeline: number }
    >();
    for (const o of opps) {
      ...
      if (o.stage === "won") b.won += num(o.amount);
      if (o.stage === "lost") b.lost += num(o.amount);
      byMonth.set(k, b);
    }
    ...
    return {
      ...
      chart: {
        ...
        format: "currency",
        currency: "USD",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.month, value: r.opened })),
      },
    };
  },
};
...
export const campaignEfficiencyReport: ReportDefinition = {
  ...
  async execute({ filters, resources }): Promise<ReportResult> {
    ...
    return {
      ...
      chart: {
        ...
        format: "percent",
        from: (rs) => (rs as typeof rows).slice(0, 12).map((r) => ({ label: r.name, value: r.roi })),
      },
    };
  },
};
...
export const lostOpportunityReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    return {
      ...
      chart: {
        ...
        format: "currency",
        currency: "USD",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.reason, value: r.value })),
      },
    };
  },
};
...
export const firstResponseTimeReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const byOwner = new Map<string, { owner: string; count: number; totalMin: number; avgMin: number }>();
    for (const l of responded) {
      const created = parseDate(l.createdAt);
      const resp = parseDate(l.firstResponseAt);
      if (!created || !resp) continue;
    ...
    return {
      ...
      chart: {
        ...
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.owner, value: r.avgMin })),
      },
    };
  },
};
...
export const CRM_REPORTS: readonly ReportDefinition[] = [
  leadDetailsReport,
...
export const crmReportsIndexView = defineCustomView({
  ...
  render: () => (
    <div className="flex flex-col gap-4">
      <PageHeader
        title="CRM Reports"
        description="Lead conversion, pipeline health, campaign ROI, win-loss analysis — all live from the record set."
      />
      <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        {CRM_REPORTS.map((r) => (
          <Card
            ...
            onClick={() => (window.location.hash = `/crm/reports/${r.id}`)}
          >
    ...
    if (!report) {
      return (
        <EmptyStateFramework
          ...
          description={`No CRM report with id "${id}".`}
          primary={{ label: "Back to reports", href: "/crm/reports" }}
        />
      );
    }
