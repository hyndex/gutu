...
export type ContactStage = typeof STAGES[number]["id"];
...
export interface RelationshipEntity {
  id: string;
  label: string;
  kind: "company" | "person" | "vendor" | "partner";
}
...
export interface RelationshipEdge {
  ...
  from: string;
  to: string;
  kind: "employs" | "partner" | "vendor" | "customer" | "subsidiary";
  strength: number;
}
...
export const CONTACTS: Contact[] = Array.from({ length: 44 }, (_, i) => {
...
export const QUOTES: Quote[] = Array.from({ length: 18 }, (_, i) => ({
  id: `q_${800 + i}`,
  number: `Q-${3000 + i}`,
  account: pick(COMPANIES, i + 4).name,
  amount: 4_000 + ((i * 7919) % 60_000),
  status: pick(["draft", "sent", "accepted", "expired"] as const, i),
  expiresAt: daysAgoIso(-7 + (i * 3) % 60),
}));

/* Activity ---------------------------------------------------------------- */

export const ACTIVITIES: ActivityItem[] = Array.from({ length: 48 }, (_, i) => {
  const kind = (["email", "call", "meeting", "note", "task"] as const)[i % 5];
  const contact = CONTACTS[i % CONTACTS.length];
...
export const POSTS: CommunityPost[] = Array.from({ length: 24 }, (_, i) => {
...
export const MODERATION: ModerationItem[] = Array.from({ length: 7 }, (_, i) => ({
  id: `m_${3000 + i}`,
  reportedBy: pick(REP_NAMES, i),
  target: `Post #${2000 + (i * 3) % 24}`,
  reason: pick(
    ["Spam", "Off-topic", "Abusive language", "Personal attack", "Low-effort post"],
    i,
  ),
  severity: pick(["low", "medium", "high"] as const, i),
  reportedAt: hoursAgoIso(i * 7 + 2),
  status: pick(["open", "open", "actioned", "dismissed"] as const, i),
}));
...
const KINDS = ["employs", "partner", "vendor", "customer"] as const;
...
export function dealStageIntent(id: string) {
  return DEAL_STAGES.find((s) => s.id === id)?.intent ?? "neutral";
}
export function stageIntent(id: string) {
