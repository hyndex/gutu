...
const topSpacesReport: ReportDefinition = {
...
const topAuthorsReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    const by = new Map<string, { author: string; posts: number; likes: number; replies: number }>();
...
const moderationReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    const by = new Map<string, { severity: string; open: number; actioned: number; dismissed: number }>();
    for (const r of reports) {
      const s = str(r.severity);
      const row = by.get(s) ?? { severity: s, open: 0, actioned: 0, dismissed: 0 };
      if (r.status === "open") row.open++;
      else if (r.status === "actioned") row.actioned++;
      else row.dismissed++;
      by.set(s, row);
    }
...
export const COMMUNITY_REPORTS: readonly ReportDefinition[] = [
