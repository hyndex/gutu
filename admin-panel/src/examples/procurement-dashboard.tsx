...
const supplierSpendReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const by = new Map<string, { supplier: string; pos: number; spend: number; avg: number }>();
    for (const p of pos) {
    ...
    return {
      ...
      chart: { kind: "bar", label: "Spend", format: "currency", currency: "USD",
        from: (rs) => (rs as typeof rows).slice(0, 10).map((r) => ({ label: r.supplier, value: r.spend })) },
    };
  },
};

const poAgingReport: ReportDefinition = {
...
const supplierPerformanceReport: ReportDefinition = {
...
const requisitionReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    const by = new Map<string, { status: string; count: number; value: number }>();
    for (const r of reqs) {
...
const rfqReport: ReportDefinition = {
...
const savingsReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const pos = await fetchAll(resources, "procurement.purchase-order");
    const by = new Map<string, { supplier: string; listTotal: number; paid: number; saved: number; rate: number }>();
    for (const p of pos) {
      const s = str(p.vendor);
...
export const PROCUREMENT_REPORTS: readonly ReportDefinition[] = [
