...
const byCategoryReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const by = new Map<string, { category: string; total: number; active: number; totalPrice: number; avgPrice: number }>();
    for (const p of products) {
      const c = str(p.category);
      const r = by.get(c) ?? { category: c, total: 0, active: 0, totalPrice: 0, avgPrice: 0 };
      r.total++;
      if (p.status === "active") r.active++;
...
const priceDistReport: ReportDefinition = {
...
const newProductsReport: ReportDefinition = {
