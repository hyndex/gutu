...
const revenueReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const by = new Map<string, { service: string; count: number; revenue: number }>();
    ...
    return {
      ...
      chart: { kind: "bar", label: "Revenue", format: "currency", currency: "USD",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.service, value: r.revenue })) },
    };
  },
};

const utilizationReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const bookings = await fetchAll(resources, "booking.booking");
    const by = new Map<string, { staff: string; bookings: number; minutes: number; hours: number }>();
    for (const b of bookings) {
...
const cancellationReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const bookings = await fetchAll(resources, "booking.booking");
    const by = new Map<string, { service: string; total: number; cancelled: number; rate: number }>();
    for (const b of bookings) {
      const s = str(b.service);
      const r = by.get(s) ?? { service: s, total: 0, cancelled: 0, rate: 0 };
      r.total++;
      if (b.status === "cancelled") r.cancelled++;
      by.set(s, r);
    }
    const rows = [...by.values()]
...
const waitlistConversionReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    const by = new Map<string, { status: string; count: number }>();
    for (const w of wl) {
    ...
    return {
      ...
      chart: { kind: "donut", label: "Waitlist outcomes",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.status, value: r.count })) },
    };
  },
};

const customerFrequencyReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const bookings = await fetchAll(resources, "booking.booking");
    const by = new Map<string, { customer: string; bookings: number; spend: number }>();
    for (const b of bookings) {
      const c = str(b.customer);
      const r = by.get(c) ?? { customer: c, bookings: 0, spend: 0 };
      r.bookings++;
      if (b.status === "completed") r.spend += num(b.amount);
      by.set(c, r);
    }
...
export const BOOKING_REPORTS: readonly ReportDefinition[] = [
  revenueReport,
  utilizationReport,
  cancellationReport,
