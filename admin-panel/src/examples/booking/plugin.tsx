...
(bookingResource as unknown as {
  __seed: readonly Record<string, unknown>[];
}).__seed = seed();
  ...
  actions: [
    ...
    },
    {
      ...
      run: async ({ records, resource, runtime }) => {
        await Promise.all(
          records.map((r) =>
            runtime.update(resource, String(r.id), { status: "confirmed" }),
          ),
        );
        runtime.toast({
    ...
    },
    {
      ...
      run: async ({ records, resource, runtime }) => {
        await Promise.all(
          records.map((r) =>
            runtime.update(resource, String(r.id), { status: "cancelled" }),
          ),
        );
  ...
  tabs: [
    {
      ...
      render: (r) => (
        <dl className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
          ...
          <Row label="Starts" value={formatDateTime(r.startAt as string)} />
          <Row label="Duration" value={`${r.durationMin} min`} />
          <Row
            label="Amount"
    ...
    },
    {
      ...
      render: () => (
        <EmptyState
          ...
          description="Lifecycle events will appear here as the booking progresses."
        />
      ),
    },
  ],
  ...
  widgets: [
    {
      ...
      title: "",
      size: "sm",
      render: () => <BookingDashboardKpis kind="today" />,
    },
    {
      id: "kpi-week",
      title: "",
      size: "sm",
      render: () => <BookingDashboardKpis kind="week" />,
    },
    {
      id: "kpi-revenue",
      title: "",
      size: "sm",
      render: () => <BookingDashboardKpis kind="revenue" />,
    },
    {
      id: "kpi-cancel",
      title: "",
      size: "sm",
      render: () => <BookingDashboardKpis kind="cancel" />,
    },
    {
      id: "intro",
      ...
      render: () => (
        <div className="text-sm text-text-secondary leading-relaxed">
          <p className="mb-2">
            This dashboard is contributed by the <code className="font-mono text-xs px-1 rounded bg-surface-2">booking</code> plugin.
            Every widget, nav entry, view, filter, action, and command in this admin
            is declaratively provided by a plugin — no JSX was written in the shell.
          </p>
          <p>
            Try <kbd className="text-xs font-mono px-1 rounded bg-surface-2 border border-border">⌘K</kbd> to open the command palette and navigate across plugins, or hit <b>New booking</b> to exercise the form renderer.
          </p>
        </div>
      ),
    },
...
function Row({ label, value }: { label: string; value: unknown }) {
  return (
    <>
      ...
      <dd className="text-text-primary">{value as React.ReactNode}</dd>
    </>
  );
}
