...
const usersByRoleReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const by = new Map<string, { role: string; total: number; mfaEnrolled: number; active: number }>();
    for (const u of users) {
      const r = str(u.role);
      const row = by.get(r) ?? { role: r, total: 0, mfaEnrolled: 0, active: 0 };
      row.total++;
      if (u.mfa) row.mfaEnrolled++;
      if (u.status === "active") row.active++;
      by.set(r, row);
    }
...
const signinHistoryReport: ReportDefinition = {
...
const inactiveUsersReport: ReportDefinition = {
...
const tokensReport: ReportDefinition = {
...
export const AUTH_REPORTS: readonly ReportDefinition[] = [
