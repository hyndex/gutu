...
const queueHealthReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const by = new Map<string, { queue: string; total: number; failed: number; avgDuration: number; sum: number }>();
    for (const j of jobs) {
      const q = str(j.queue);
      const r = by.get(q) ?? { queue: q, total: 0, failed: 0, avgDuration: 0, sum: 0 };
      r.total++;
      if (j.severity === "error") r.failed++;
