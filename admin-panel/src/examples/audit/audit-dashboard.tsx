...
const topActorsReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const by = new Map<string, { actor: string; total: number; errors: number }>();
...
const actionBreakdownReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const events = await fetchAll(resources, "audit.event");
    const by = new Map<string, { action: string; count: number; level: string }>();
    for (const e of events) {
...
const failedAuthReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const events = await fetchAll(resources, "audit.event");
    const by = new Map<string, { actor: string; ip: string; failures: number; lastAt: string }>();
    for (const e of events) {
      if (e.action !== "auth.failed" && e.level !== "error") continue;
      const k = `${str(e.actor)}|${str(e.ip)}`;
      const r = by.get(k) ?? { actor: str(e.actor), ip: str(e.ip), failures: 0, lastAt: "" };
      r.failures++;
      if (str(e.occurredAt) > r.lastAt) r.lastAt = str(e.occurredAt);
      by.set(k, r);
    }
...
const complianceReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    const by = new Map<string, { action: string; count: number }>();
    for (const e of events) {
      if (!tracked.includes(str(e.action))) continue;
      const a = str(e.action);
...
export const AUDIT_REPORTS: readonly ReportDefinition[] = [
