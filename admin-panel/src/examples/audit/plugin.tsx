...
(eventResource as unknown as { __seed: Record<string, unknown>[] }).__seed = seedEvents();
  ...
  columns: [
    ...
    },
    {
      ...
      render: (v) => (
        <Badge
          intent={
            v === "error" ? "danger" : v === "warn" ? "warning" : "info"
          }
        >
          {String(v)}
        </Badge>
    ...
    { field: "resource", label: "Resource", width: 160 },
    {
      ...
      width: 140,
      render: (v) => (
        <code className="text-xs font-mono text-text-muted">{String(v)}</code>
      ),
    },
  ],
  filters: [
