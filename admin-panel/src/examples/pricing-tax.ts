...
const priceSpreadReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const by = new Map<string, { currency: string; min: number; max: number; avg: number; count: number; total: number }>();
...
const discountUsageReport: ReportDefinition = {
...
const taxByRegionReport: ReportDefinition = {
