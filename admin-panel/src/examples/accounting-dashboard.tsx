...
const arAgingReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    for (const i of inv) {
      ...
      if (idx >= 0) { rows[idx].count++; rows[idx].outstanding += num(i.amount); }
    }
    return {
      columns: [
...
const apAgingReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    for (const b of bills) {
      ...
      if (idx >= 0) { rows[idx].count++; rows[idx].owed += num(b.amount); }
    }
    return {
      columns: [
        { field: "bucket", label: "Bucket", fieldtype: "text" },
...
const invoiceTrendsReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const inv = await fetchAll(resources, "accounting.invoice");
    const byMonth = new Map<string, { month: string; count: number; billed: number; collected: number }>();
    for (const i of inv) {
      const d = i.issuedAt ? new Date(str(i.issuedAt)) : null;
      if (!d || Number.isNaN(d.getTime())) continue;
      const k = monthKey(d);
      const b = byMonth.get(k) ?? { month: k, count: 0, billed: 0, collected: 0 };
      b.count++; b.billed += num(i.amount);
      if (i.status === "paid") b.collected += num(i.amount);
      byMonth.set(k, b);
    }
    ...
    return {
      ...
      chart: { kind: "line", label: "Billed vs collected", format: "currency", currency: "USD",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.month, value: r.collected })) },
    };
  },
};

const budgetVarianceReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    return {
      ...
      chart: { kind: "bar", label: "Variance % by department", format: "percent",
        from: (rs2) => (rs2 as typeof rows).map((r) => ({ label: r.department, value: r.variancePct })) },
    };
  },
};
...
const cashFlowReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    const byMonth = new Map<string, { month: string; in: number; out: number; net: number }>();
    for (const p of pays) {
      const d = p.postedAt ? new Date(str(p.postedAt)) : null;
      if (!d || Number.isNaN(d.getTime())) continue;
      const k = monthKey(d);
      const b = byMonth.get(k) ?? { month: k, in: 0, out: 0, net: 0 };
      if (p.direction === "receive") b.in += num(p.amount);
      else b.out += num(p.amount);
      b.net = b.in - b.out;
      byMonth.set(k, b);
    }
    const rows = [...byMonth.values()].sort((a, b) => a.month.localeCompare(b.month));
    return {
      ...
      chart: { kind: "line", label: "Net cash flow", format: "currency", currency: "USD",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.month, value: r.net })) },
    };
  },
};

const taxSummaryReport: ReportDefinition = {
...
export const ACCOUNTING_REPORTS: readonly ReportDefinition[] = [
