...
const stockBalanceReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    return {
      ...
      chart: {
        ...
        from: (rs) => (rs as typeof rows).slice(0, 10).map((r) => ({ label: r.sku, value: r.value })),
      },
    };
  },
};

const stockLedgerReport: ReportDefinition = {
...
const stockAgingReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    return {
      ...
      chart: {
        ...
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.bucket, value: r.value })),
      },
    };
  },
};

const stockProjectedReport: ReportDefinition = {
...
const warehouseBalanceReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const bins = await fetchAll(resources, "inventory.bin");
    const by = new Map<string, { warehouse: string; skus: number; units: number; value: number }>();
    for (const b of bins) {
    ...
    return {
      ...
      chart: {
        ...
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.warehouse, value: r.value })),
      },
    };
  },
};

const reorderReport: ReportDefinition = {
...
const batchExpiryReport: ReportDefinition = {
...
const itemShortageReport: ReportDefinition = {
...
const stockValuationReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const items = await fetchAll(resources, "inventory.item");
    const by = new Map<string, { category: string; skus: number; units: number; value: number }>();
    for (const i of items) {
    ...
    return {
      ...
      chart: {
        ...
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.category, value: r.value })),
      },
    };
  },
};
