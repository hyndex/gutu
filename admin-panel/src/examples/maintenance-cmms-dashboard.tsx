...
const workOrderReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const by = new Map<string, { type: string; total: number; open: number; avgHrs: number; sumHrs: number; completed: number }>();
    for (const w of wos) {
      ...
      if (w.status === "resolved" || w.status === "closed") {
        ...
        if (!Number.isNaN(c) && !Number.isNaN(f)) r.sumHrs += (f - c) / 3_600_000;
      } else r.open++;
      by.set(t, r);
    }
...
const pmComplianceReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    const by = new Map<string, { asset: string; scheduled: number; onTime: number; missed: number; rate: number }>();
    for (const s of schedules) {
      const a = str(s.asset);
      const r = by.get(a) ?? { asset: a, scheduled: 0, onTime: 0, missed: 0, rate: 0 };
      r.scheduled++;
      if (s.lastCompletedOnTime) r.onTime++;
      if (s.missed) r.missed++;
      by.set(a, r);
    }
...
const downtimeReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    const by = new Map<string, { asset: string; incidents: number; hours: number; costImpact: number }>();
...
const reliabilityReport: ReportDefinition = {
...
const technicianProductivityReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const wos = await fetchAll(resources, "maintenance-cmms.work-order");
    const by = new Map<string, { technician: string; completed: number; hours: number }>();
    for (const w of wos) {
...
export const CMMS_REPORTS: readonly ReportDefinition[] = [
