...
const passRateReport: ReportDefinition = {
  ...
  async execute({ resources }): Promise<ReportResult> {
    ...
    const by = new Map<string, { product: string; total: number; passed: number; failed: number; rate: number }>();
    for (const ins of inspections) {
      const p = str(ins.product);
      const r = by.get(p) ?? { product: p, total: 0, passed: 0, failed: 0, rate: 0 };
      r.total++;
      if (ins.result === "pass") r.passed++;
      else if (ins.result === "fail") r.failed++;
      by.set(p, r);
    }
...
const ncrAgingReport: ReportDefinition = {
...
const defectParetoReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    const by = new Map<string, { defectType: string; count: number; cost: number }>();
    ...
    return {
      ...
      chart: { kind: "bar", label: "Defects by type",
        from: (rs) => (rs as typeof rows).map((r) => ({ label: r.defectType, value: r.count })) },
    };
  },
};

const capaEffectivenessReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    const by = new Map<string, { status: string; count: number; verified: number }>();
    for (const c of capas) {
      const s = str(c.status);
      const r = by.get(s) ?? { status: s, count: 0, verified: 0 };
      r.count++;
      if (c.verified) r.verified++;
      by.set(s, r);
    }
    const rows = [...by.values()].sort((a, b) => b.count - a.count);
    return {
...
const supplierQualityReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    const defects = await fetchAll(resources, "quality.defect");
    const by = new Map<string, { supplier: string; defects: number; cost: number }>();
    for (const d of defects) {
...
const auditFindingsReport: ReportDefinition = {
  ...
  async execute({ resources }) {
    ...
    const by = new Map<string, { severity: string; total: number; open: number; closed: number }>();
    for (const a of audits) {
      const s = str(a.severity);
      const r = by.get(s) ?? { severity: s, total: 0, open: 0, closed: 0 };
      r.total++;
      if (a.status === "closed") r.closed++;
      else r.open++;
      by.set(s, r);
    }
...
export const QUALITY_REPORTS: readonly ReportDefinition[] = [
