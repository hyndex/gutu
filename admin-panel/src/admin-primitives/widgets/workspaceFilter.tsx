...
/** The full filter state exposed to widgets: a map keyed by resource name
 *  (with `"*"` for the always-on bucket) whose values are AND-merged leaves.
 *  A widget picks up `"*"` + its own resource's leaves and AND-merges the
 *  result into its spec. */
export interface WorkspaceFilterState {
  /** Leaves that apply to every widget (fields declared without `appliesTo`
   *  or with `fallbackAll: true`). */
  readonly all?: FilterTree;
  /** Leaves scoped to a specific `aggregation.resource` / `QuickList.resource`. */
  readonly byResource: Readonly<Record<string, FilterTree | undefined>>;
}

export const WorkspaceFilterContext = React.createContext<WorkspaceFilterState | undefined>(
  undefined,
);
...
export function useWorkspaceFilter(resource?: string): FilterTree | undefined {
...
export function buildFilterState(
...
): WorkspaceFilterState | undefined {
  const allLeaves: FilterTree[] = [];
  const byResourceLeaves: Record<string, FilterTree[]> = {};
  ...
  for (const f of fields) {
    ...
    if (f.fallbackAll) allLeaves.push(leaf);
  }

  const hasAll = allLeaves.length > 0;
  const resources = Object.keys(byResourceLeaves);
  if (!hasAll && resources.length === 0) return undefined;

  const all = hasAll ? collapse(allLeaves) : undefined;
  const byResource: Record<string, FilterTree | undefined> = {};
  for (const r of resources) byResource[r] = collapse(byResourceLeaves[r]);

  return { all, byResource };
}

function collapse(leaves: FilterTree[]): FilterTree | undefined {
  if (leaves.length === 0) return undefined;
  if (leaves.length === 1) return leaves[0];
  return { and: leaves };
}

function toLeaf(f: WorkspaceFilterField, v: unknown): FilterTree | null {
  ...
  if (f.kind === "date-range" && Array.isArray(v) && v.length === 2) {
    ...
    if (from && to) return { field: f.field, op: "between", value: [from, to] };
    if (from) return { field: f.field, op: "gte", value: from };
    if (to) return { field: f.field, op: "lte", value: to };
  }
  return null;
}
