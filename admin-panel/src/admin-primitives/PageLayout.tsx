...
export function PageGrid({
  ...
  return <div className={cn("grid gap-3", map[columns], className)}>{children}</div>;
}
...
export function SplitLayout({
  ...
  className?: string;
  children: React.ReactNode;
  /** Accepts CSS grid-template-columns-style ratios, e.g. "2fr_1fr". */
  ratio?: string;
  gap?: string;
}) {
  ...
  return (
    ...
    >
      <div className={cn("grid", gap)} style={{ gridTemplateColumns: template }}>
        {children}
      </div>
    </div>
  );
...
export function Section({
  ...
  return (
    <section
      className={cn(
        !bare && "rounded-lg border border-border bg-surface-0 shadow-xs",
        className,
      )}
    >
      {(title || actions) && (
