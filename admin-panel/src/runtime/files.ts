...
export async function uploadFile(
  file: File,
  opts: { resource?: string; recordId?: string } = {},
): Promise<UploadedFile> {
  const form = new FormData();
  form.append("file", file);
  if (opts.resource) form.append("resource", opts.resource);
...
export function useRecordFiles(resource: string, recordId: string | undefined) {
  ...
  const reload = React.useCallback(() => {
    ...
    fetch(`${BASE}/files?${q}`, {
      ...
      .then(async (r) => {
        ...
        return (await r.json()) as { rows: UploadedFile[] };
      })
