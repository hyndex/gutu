...
export class MockBackend implements ResourceAdapter {
  ...
  seed(resource: string, rows: readonly Record<string, unknown>[]): void {
  ...
  private ensure(resource: string) {
    ...
    if (query.filters) {
      for (const [field, value] of Object.entries(query.filters)) {
        ...
        if (Array.isArray(value)) {
          if (value.length > 0) rows = rows.filter((r) => value.includes(r[field]));
          continue;
        }
        if (typeof value === "object" && "from" in (value as object)) {
  ...
  async update(resource: string, id: string, data: Record<string, unknown>) {
    ...
    if (!existing) throw new Error(`[mock] ${resource}/${id} not found`);
  ...
  async delete(resource: string, id: string) {
    await sleep(this.latency);
    const store = this.ensure(resource);
    if (!store.delete(id)) throw new Error(`[mock] ${resource}/${id} not found`);
  }
}
