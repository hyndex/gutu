...
- You need **audit evidence**, **sensitive actions**, **accountability** as a governed domain boundary.
...
| `framework/builtin-plugins/audit-core` | Nested publishable plugin package. |
| `framework/builtin-plugins/audit-core/src` | Runtime source, actions, resources, services, and UI exports. |
| `framework/builtin-plugins/audit-core/tests` | Unit, contract, integration, and migration coverage where present. |
| `framework/builtin-plugins/audit-core/docs` | Internal domain-doc source set kept in sync with this guide. |
| `framework/builtin-plugins/audit-core/db/schema.ts` | Database schema contract when durable state is owned. |
| `framework/builtin-plugins/audit-core/src/postgres.ts` | SQL migration and rollback helpers when exported. |

## Manifest Contract

| Field | Value |
| --- | --- |
| Package Name | `@plugins/audit-core` |
| Manifest ID | `audit-core` |
| Display Name | Audit Core |
| Version | `0.1.0` |
| Kind | `app` |
...
| --- | --- | --- | --- |
| Action | `audit.events.record` | Permission: `audit.events.record` | Idempotent<br>Audited |
| Resource | `audit.events` | Portal disabled | Admin auto-CRUD enabled<br>Fields: `actionId`, `actorId`, `severity`, `status`, `createdAt` |
...
- Database compatibility: `postgres`, `sqlite`
- Schema file: `framework/builtin-plugins/audit-core/db/schema.ts`
- SQL helper file: `framework/builtin-plugins/audit-core/src/postgres.ts`
...
```mermaid
flowchart LR
  caller["Host or operator"] --> action["audit.events.record"]
  action --> validation["Schema + permission guard"]
  validation --> service["Audit Core service layer"]
  service --> state["audit.events"]
  state --> ui["UI surface"]
```
...
```ts
import { manifest, recordAuditEventAction } from "@plugins/audit-core";

console.log("plugin", manifest.id);
console.log("action", recordAuditEventAction.id);
```
