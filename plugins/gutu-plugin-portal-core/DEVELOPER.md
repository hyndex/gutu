...
- You need **portal shell**, **self-service entrypoints**, **portal-aware resources** as a governed domain boundary.
...
| `framework/builtin-plugins/portal-core` | Nested publishable plugin package. |
| `framework/builtin-plugins/portal-core/src` | Runtime source, actions, resources, services, and UI exports. |
| `framework/builtin-plugins/portal-core/tests` | Unit, contract, integration, and migration coverage where present. |
| `framework/builtin-plugins/portal-core/docs` | Internal domain-doc source set kept in sync with this guide. |
| `framework/builtin-plugins/portal-core/db/schema.ts` | Database schema contract when durable state is owned. |
| `framework/builtin-plugins/portal-core/src/postgres.ts` | SQL migration and rollback helpers when exported. |

## Manifest Contract

| Field | Value |
| --- | --- |
| Package Name | `@plugins/portal-core` |
| Manifest ID | `portal-core` |
| Display Name | Portal Core |
| Version | `0.1.0` |
| Kind | `app` |
...
| Requested Capabilities | `ui.register.admin`, `api.rest.mount`, `data.write.portal` |
| Provides Capabilities | `portal.accounts` |
| Owns Data | `portal.accounts` |
...
| --- | --- | --- | --- |
| Action | `portal.accounts.enable` | Permission: `portal.accounts.enable` | Idempotent<br>Audited |
| Resource | `portal.accounts` | Portal enabled | Admin auto-CRUD enabled<br>Fields: `accountType`, `createdAt`, `homeRoute`, `membershipStatus`, `subjectId` |
...
- Database compatibility: `postgres`, `sqlite`
- Schema file: `framework/builtin-plugins/portal-core/db/schema.ts`
- SQL helper file: `framework/builtin-plugins/portal-core/src/postgres.ts`
...
```mermaid
flowchart LR
  caller["Host or operator"] --> action["portal.accounts.enable"]
  action --> validation["Schema + permission guard"]
  validation --> service["Portal Core service layer"]
  service --> state["portal.accounts"]
  state --> ui["UI surface"]
```
...
```ts
import { manifest, enablePortalAccountAction, PortalAccountResource, uiSurface } from "@plugins/portal-core";

export const pluginSurface = {
  manifest,
  enablePortalAccountAction,
  PortalAccountResource,
...
```ts
import { manifest, enablePortalAccountAction } from "@plugins/portal-core";

console.log("plugin", manifest.id);
console.log("action", enablePortalAccountAction.id);
```
...
- Exports 1 governed action: `portal.accounts.enable`.
- Owns 1 resource contract: `portal.accounts`.
