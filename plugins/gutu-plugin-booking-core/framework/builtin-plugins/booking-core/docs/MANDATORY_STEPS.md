...
2. Keep the nested docs under `framework/builtin-plugins/booking-core/docs` synchronized with the same truth.
3. Run the repo-local verification commands:
   ...
   - `bun run test`
   - `bun run docs:check`
4. Run any extra lanes present for this plugin: `test:unit`, `test:contracts`, `test:migrations`.
...
1. Depend on the manifest ID `booking-core` and the package import `@plugins/booking-core`.
