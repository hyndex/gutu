...
- You need **retrieval**, **memory collections**, **grounding diagnostics** as a governed domain boundary.
...
| `framework/builtin-plugins/ai-rag` | Nested publishable plugin package. |
| `framework/builtin-plugins/ai-rag/src` | Runtime source, actions, resources, services, and UI exports. |
| `framework/builtin-plugins/ai-rag/tests` | Unit, contract, integration, and migration coverage where present. |
| `framework/builtin-plugins/ai-rag/docs` | Internal domain-doc source set kept in sync with this guide. |
| `framework/builtin-plugins/ai-rag/db/schema.ts` | Database schema contract when durable state is owned. |
| `framework/builtin-plugins/ai-rag/src/postgres.ts` | SQL migration and rollback helpers when exported. |
...
```mermaid
flowchart LR
  caller["Host or operator"] --> action["ai.memory.ingest"]
  action --> validation["Schema + permission guard"]
  validation --> service["AI RAG service layer"]
  service --> state["ai.memory-collections"]
...
```ts
import { manifest, ingestMemoryDocumentAction } from "@plugins/ai-rag";

console.log("plugin", manifest.id);
console.log("action", ingestMemoryDocumentAction.id);
```
