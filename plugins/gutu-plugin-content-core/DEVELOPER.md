...
- You need **pages**, **posts**, **content types** as a governed domain boundary.
...
| `framework/builtin-plugins/content-core` | Nested publishable plugin package. |
| `framework/builtin-plugins/content-core/src` | Runtime source, actions, resources, services, and UI exports. |
| `framework/builtin-plugins/content-core/tests` | Unit, contract, integration, and migration coverage where present. |
| `framework/builtin-plugins/content-core/docs` | Internal domain-doc source set kept in sync with this guide. |
| `framework/builtin-plugins/content-core/db/schema.ts` | Database schema contract when durable state is owned. |
| `framework/builtin-plugins/content-core/src/postgres.ts` | SQL migration and rollback helpers when exported. |

## Manifest Contract

| Field | Value |
| --- | --- |
| Package Name | `@plugins/content-core` |
| Manifest ID | `content-core` |
| Display Name | Content Core |
| Version | `0.1.0` |
| Kind | `app` |
...
| Provides Capabilities | `content.entries` |
| Owns Data | `content.entries` |
...
| --- | --- | --- | --- |
| Action | `content.entries.publish` | Permission: `content.entries.publish` | Idempotent<br>Audited |
| Resource | `content.entries` | Portal disabled | Admin auto-CRUD enabled<br>Fields: `label`, `status`, `createdAt` |
...
- Database compatibility: `postgres`, `sqlite`
- Schema file: `framework/builtin-plugins/content-core/db/schema.ts`
- SQL helper file: `framework/builtin-plugins/content-core/src/postgres.ts`
...
```mermaid
flowchart LR
  caller["Host or operator"] --> action["content.entries.publish"]
  action --> validation["Schema + permission guard"]
  validation --> service["Content Core service layer"]
  service --> state["content.entries"]
  state --> ui["UI surface"]
```
...
```ts
import { manifest, publishContentEntryAction, ContentEntryResource, uiSurface } from "@plugins/content-core";
...
```ts
import { manifest, publishContentEntryAction } from "@plugins/content-core";

console.log("plugin", manifest.id);
console.log("action", publishContentEntryAction.id);
```
...
- Exports 1 governed action: `content.entries.publish`.
- Owns 1 resource contract: `content.entries`.
