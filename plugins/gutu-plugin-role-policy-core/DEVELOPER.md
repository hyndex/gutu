# Role Policy Core Developer Guide
...
- You need **roles**, **policies**, **access governance** as a governed domain boundary.
...
| `framework/builtin-plugins/role-policy-core` | Nested publishable plugin package. |
| `framework/builtin-plugins/role-policy-core/src` | Runtime source, actions, resources, services, and UI exports. |
| `framework/builtin-plugins/role-policy-core/tests` | Unit, contract, integration, and migration coverage where present. |
| `framework/builtin-plugins/role-policy-core/docs` | Internal domain-doc source set kept in sync with this guide. |
| `framework/builtin-plugins/role-policy-core/db/schema.ts` | Database schema contract when durable state is owned. |
| `framework/builtin-plugins/role-policy-core/src/postgres.ts` | SQL migration and rollback helpers when exported. |

## Manifest Contract

| Field | Value |
| --- | --- |
| Package Name | `@plugins/role-policy-core` |
| Manifest ID | `role-policy-core` |
| Display Name | Role Policy Core |
| Version | `0.1.0` |
| Kind | `app` |
...
| Provides Capabilities | `roles.grants` |
| Owns Data | `roles.grants` |
...
| --- | --- | --- | --- |
| Action | `roles.grants.assign` | Permission: `roles.grants.assign` | Idempotent<br>Audited |
| Resource | `roles.grants` | Portal disabled | Admin auto-CRUD enabled<br>Fields: `subjectType`, `permission`, `effect`, `status`, `createdAt` |
...
- Database compatibility: `postgres`, `sqlite`
- Schema file: `framework/builtin-plugins/role-policy-core/db/schema.ts`
- SQL helper file: `framework/builtin-plugins/role-policy-core/src/postgres.ts`
...
```mermaid
flowchart LR
  caller["Host or operator"] --> action["roles.grants.assign"]
  action --> validation["Schema + permission guard"]
  validation --> service["Role Policy Core service layer"]
  service --> state["roles.grants"]
  state --> ui["UI surface"]
```
...
```ts
import { manifest, assignGrantAction, GrantResource, uiSurface } from "@plugins/role-policy-core";
...
```ts
import { manifest, assignGrantAction } from "@plugins/role-policy-core";

console.log("plugin", manifest.id);
console.log("action", assignGrantAction.id);
```
