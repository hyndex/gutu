# Org Tenant Core Developer Guide
...
- You need **tenant graph**, **organization graph**, **isolation boundaries** as a governed domain boundary.
...
| `framework/builtin-plugins/org-tenant-core` | Nested publishable plugin package. |
| `framework/builtin-plugins/org-tenant-core/src` | Runtime source, actions, resources, services, and UI exports. |
| `framework/builtin-plugins/org-tenant-core/tests` | Unit, contract, integration, and migration coverage where present. |
| `framework/builtin-plugins/org-tenant-core/docs` | Internal domain-doc source set kept in sync with this guide. |
| `framework/builtin-plugins/org-tenant-core/db/schema.ts` | Database schema contract when durable state is owned. |
| `framework/builtin-plugins/org-tenant-core/src/postgres.ts` | SQL migration and rollback helpers when exported. |

## Manifest Contract

| Field | Value |
| --- | --- |
| Package Name | `@plugins/org-tenant-core` |
| Manifest ID | `org-tenant-core` |
| Display Name | Org Tenant Core |
| Version | `0.1.0` |
| Kind | `app` |
...
| --- | --- | --- | --- |
| Action | `org.tenants.activate` | Permission: `org.tenants.activate` | Idempotent<br>Audited |
| Resource | `org.tenants` | Portal disabled | Admin auto-CRUD enabled<br>Fields: `slug`, `displayName`, `billingPlan`, `status`, `createdAt` |
...
- Database compatibility: `postgres`, `sqlite`
- Schema file: `framework/builtin-plugins/org-tenant-core/db/schema.ts`
- SQL helper file: `framework/builtin-plugins/org-tenant-core/src/postgres.ts`
...
```mermaid
flowchart LR
  caller["Host or operator"] --> action["org.tenants.activate"]
  action --> validation["Schema + permission guard"]
  validation --> service["Org Tenant Core service layer"]
  service --> state["org.tenants"]
  state --> ui["UI surface"]
```
...
```ts
import { manifest, activateTenantAction, TenantResource, uiSurface } from "@plugins/org-tenant-core";
...
```ts
import { manifest, activateTenantAction } from "@plugins/org-tenant-core";

console.log("plugin", manifest.id);
console.log("action", activateTenantAction.id);
```
...
- Exports 1 governed action: `org.tenants.activate`.
- Owns 1 resource contract: `org.tenants`.
