...
- You need **search indexes**, **query contracts**, **typed retrieval** as a governed domain boundary.
...
| `framework/builtin-plugins/search-core` | Nested publishable plugin package. |
| `framework/builtin-plugins/search-core/src` | Runtime source, actions, resources, services, and UI exports. |
| `framework/builtin-plugins/search-core/tests` | Unit, contract, integration, and migration coverage where present. |
| `framework/builtin-plugins/search-core/docs` | Internal domain-doc source set kept in sync with this guide. |
| `framework/builtin-plugins/search-core/db/schema.ts` | Database schema contract when durable state is owned. |
| `framework/builtin-plugins/search-core/src/postgres.ts` | SQL migration and rollback helpers when exported. |

## Manifest Contract

| Field | Value |
| --- | --- |
| Package Name | `@plugins/search-core` |
| Manifest ID | `search-core` |
| Display Name | Search Core |
| Version | `0.1.0` |
| Kind | `app` |
...
| Provides Capabilities | `search.documents` |
| Owns Data | `search.documents` |
...
| --- | --- | --- | --- |
| Action | `search.documents.index` | Permission: `search.documents.index` | Idempotent<br>Audited |
| Resource | `search.documents` | Portal disabled | Admin auto-CRUD enabled<br>Fields: `label`, `status`, `createdAt` |
...
- Database compatibility: `postgres`, `sqlite`
- Schema file: `framework/builtin-plugins/search-core/db/schema.ts`
- SQL helper file: `framework/builtin-plugins/search-core/src/postgres.ts`
...
```mermaid
flowchart LR
  caller["Host or operator"] --> action["search.documents.index"]
  action --> validation["Schema + permission guard"]
  validation --> service["Search Core service layer"]
  service --> state["search.documents"]
  state --> ui["UI surface"]
```
...
```ts
import { manifest, indexSearchDocumentAction } from "@plugins/search-core";

console.log("plugin", manifest.id);
console.log("action", indexSearchDocumentAction.id);
```
