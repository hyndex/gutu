# Dashboard Core Developer Guide
...
- You need **dashboards**, **widgets**, **saved views** as a governed domain boundary.
...
| `framework/builtin-plugins/dashboard-core` | Nested publishable plugin package. |
| `framework/builtin-plugins/dashboard-core/src` | Runtime source, actions, resources, services, and UI exports. |
| `framework/builtin-plugins/dashboard-core/tests` | Unit, contract, integration, and migration coverage where present. |
| `framework/builtin-plugins/dashboard-core/docs` | Internal domain-doc source set kept in sync with this guide. |
| `framework/builtin-plugins/dashboard-core/db/schema.ts` | Database schema contract when durable state is owned. |
| `framework/builtin-plugins/dashboard-core/src/postgres.ts` | SQL migration and rollback helpers when exported. |
...
| Provides Capabilities | `dashboard.views` |
| Owns Data | `dashboard.views` |
...
| --- | --- | --- | --- |
| Action | `dashboard.views.publish` | Permission: `dashboard.views.publish` | Idempotent<br>Audited |
| Resource | `dashboard.views` | Portal disabled | Admin auto-CRUD enabled<br>Fields: `label`, `status`, `createdAt` |
...
- Database compatibility: `postgres`, `sqlite`
- Schema file: `framework/builtin-plugins/dashboard-core/db/schema.ts`
- SQL helper file: `framework/builtin-plugins/dashboard-core/src/postgres.ts`
...
```mermaid
flowchart LR
  caller["Host or operator"] --> action["dashboard.views.publish"]
  action --> validation["Schema + permission guard"]
  validation --> service["Dashboard Core service layer"]
  service --> state["dashboard.views"]
...
```ts
import { manifest, publishDashboardViewAction } from "@plugins/dashboard-core";

console.log("plugin", manifest.id);
console.log("action", publishDashboardViewAction.id);
```
