# Knowledge Core Developer Guide
...
- You need **knowledge base**, **article trees**, **managed docs** as a governed domain boundary.
...
| `framework/builtin-plugins/knowledge-core` | Nested publishable plugin package. |
| `framework/builtin-plugins/knowledge-core/src` | Runtime source, actions, resources, services, and UI exports. |
| `framework/builtin-plugins/knowledge-core/tests` | Unit, contract, integration, and migration coverage where present. |
| `framework/builtin-plugins/knowledge-core/docs` | Internal domain-doc source set kept in sync with this guide. |
| `framework/builtin-plugins/knowledge-core/db/schema.ts` | Database schema contract when durable state is owned. |
| `framework/builtin-plugins/knowledge-core/src/postgres.ts` | SQL migration and rollback helpers when exported. |

## Manifest Contract

| Field | Value |
| --- | --- |
| Package Name | `@plugins/knowledge-core` |
| Manifest ID | `knowledge-core` |
| Display Name | Knowledge Core |
| Version | `0.1.0` |
| Kind | `app` |
...
| Requested Capabilities | `ui.register.admin`, `api.rest.mount`, `data.write.knowledge` |
| Provides Capabilities | `knowledge.articles` |
| Owns Data | `knowledge.articles` |
...
| --- | --- | --- | --- |
| Action | `knowledge.articles.publish` | Permission: `knowledge.articles.publish` | Idempotent<br>Audited |
| Resource | `knowledge.articles` | Portal disabled | Admin auto-CRUD enabled<br>Fields: `label`, `status`, `createdAt` |
...
- Database compatibility: `postgres`, `sqlite`
- Schema file: `framework/builtin-plugins/knowledge-core/db/schema.ts`
- SQL helper file: `framework/builtin-plugins/knowledge-core/src/postgres.ts`
...
```mermaid
flowchart LR
  caller["Host or operator"] --> action["knowledge.articles.publish"]
  action --> validation["Schema + permission guard"]
  validation --> service["Knowledge Core service layer"]
  service --> state["knowledge.articles"]
  state --> ui["UI surface"]
```
...
```ts
import { manifest, publishKnowledgeArticleAction, KnowledgeArticleResource, uiSurface } from "@plugins/knowledge-core";
...
```ts
import { manifest, publishKnowledgeArticleAction } from "@plugins/knowledge-core";

console.log("plugin", manifest.id);
console.log("action", publishKnowledgeArticleAction.id);
```
...
- Exports 1 governed action: `knowledge.articles.publish`.
- Owns 1 resource contract: `knowledge.articles`.
