# Community Core Developer Guide
...
- You need **groups**, **memberships**, **community governance** as a governed domain boundary.
...
| `framework/builtin-plugins/community-core` | Nested publishable plugin package. |
| `framework/builtin-plugins/community-core/src` | Runtime source, actions, resources, services, and UI exports. |
| `framework/builtin-plugins/community-core/tests` | Unit, contract, integration, and migration coverage where present. |
| `framework/builtin-plugins/community-core/docs` | Internal domain-doc source set kept in sync with this guide. |
| `framework/builtin-plugins/community-core/db/schema.ts` | Database schema contract when durable state is owned. |
| `framework/builtin-plugins/community-core/src/postgres.ts` | SQL migration and rollback helpers when exported. |

## Manifest Contract

| Field | Value |
| --- | --- |
| Package Name | `@plugins/community-core` |
| Manifest ID | `community-core` |
| Display Name | Community Core |
| Version | `0.1.0` |
| Kind | `app` |
...
| Provides Capabilities | `community.memberships` |
| Owns Data | `community.memberships` |
...
| --- | --- | --- | --- |
| Action | `community.memberships.enroll` | Permission: `community.memberships.enroll` | Idempotent<br>Audited |
| Resource | `community.memberships` | Portal disabled | Admin auto-CRUD enabled<br>Fields: `label`, `status`, `createdAt` |
...
- Database compatibility: `postgres`, `sqlite`
- Schema file: `framework/builtin-plugins/community-core/db/schema.ts`
- SQL helper file: `framework/builtin-plugins/community-core/src/postgres.ts`
...
```mermaid
flowchart LR
  caller["Host or operator"] --> action["community.memberships.enroll"]
  action --> validation["Schema + permission guard"]
  validation --> service["Community Core service layer"]
  service --> state["community.memberships"]
  state --> ui["UI surface"]
```
...
```ts
import { manifest, enrollMembershipAction } from "@plugins/community-core";

console.log("plugin", manifest.id);
console.log("action", enrollMembershipAction.id);
```
